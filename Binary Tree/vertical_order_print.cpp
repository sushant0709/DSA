...
void verticalOrderPrintHelper(node* root,int d,map<int,vector<int>> &m){

	// base case
	if(root == NULL){
		return;
	}
	// push the data of root to the vector corresponding to d
	m[d].push_back(root->data);

	// recursive case 
	verticalOrderPrintHelper(root->left,d-1,m);
	verticalOrderPrintHelper(root->right,d+1,m);
}
void verticalOrderPrint(node* root){

	map<int,vector<int>> m;
	verticalOrderPrintHelper(root,0,m);
	for(auto x:m){
