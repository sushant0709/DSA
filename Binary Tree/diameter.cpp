...
pair<int,int> diameterOptimised(node* root){
	...
	auto lc = diameterOptimised(root->left);
	auto rc = diameterOptimised(root->right);

	pair<int,int> ans;
	// height for ans
	ans.first = 1 + max(lc.first,rc.first);
	// diameter for ans
	int d1 = lc.first+rc.first;
	int d2 = lc.second;
	int d3 = rc.second;
	ans.second = max(d1,max(d2,d3));
