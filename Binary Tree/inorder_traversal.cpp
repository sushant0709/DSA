...
void inorderRcursive(node* root){
	if(root == NULL){
		return;
	}
	// left child
	inorderRcursive(root->left);

	// node data
	cout<<root->data<<" ";

	// right data
	inorderRcursive(root->right);
...
void inorderIterative(node* root){
	...
	while(curr != NULL || !s.empty()){
		...
		// go the leftmost node until we get NULL node
		while(curr != NULL){
			// push curr into s
			s.push(curr);
			curr = curr->left;
		...
		// at this point we are at NULL node, we save the top of s
		node* n = s.top();
		// pop from s
		s.pop();

		cout<<n->data<<" ";
		// go to right of popped node
		curr = n->right;
	}
}
