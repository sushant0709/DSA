...
int printNodesatDistanceK(node* root,node* target,int k){
	...
	// recursive case
	int lcDistance = printNodesatDistanceK(root->left,target,k);
	if(lcDistance != -1){
		...
		// print node in right subtree
			...
			print_at_level_k(root->right,k-lcDistance-2);
		}
		return 1+lcDistance;
	}
	int rcDistance = printNodesatDistanceK(root->right,target,k);
	if(rcDistance != -1){
		...
		// print node in left subtree
			...
			print_at_level_k(root->left,k-rcDistance-2);
		}
		return 1+rcDistance;
	}
...
int main(int argc, char const *argv[]) {
	...
	int ans = printNodesatDistanceK(root,root->left->right,4);
	cout<<endl<<ans;
	#ifndef ONLINE_JUDGE
	clock_t end = clock();
