...
node* LevelOrderBuild(vector<int> arr){
	...
	// create root node from first element of arr
	...
	while(!q.empty()){
		// save front node and pop it out
		node* n = q.front();
		q.pop();
...
int main(int argc, char const *argv[]) {
	...
	vector<int> level = {1, 2, 3, 4, 5, -1, 6, -1, -1, 7, -1, -1, -1, -1, -1};
	cout<<level.size()<<endl;
	printLevelOrder(LevelOrderBuild(level));
