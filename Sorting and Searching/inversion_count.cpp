...
int merge(vector<int> &arr,int s,int e)
{
    ...
    while(i<= mid and j<=e)
    {
    	if(arr[i]<arr[j]){
    		...
    		// increase the cnt by all elements to right of current element
    		// if a smaller element found in the right subarray.
    		cnt += (mid - i + 1); 
    		temp[k++]=arr[j++];
    	}
    }
...
int main(int argc, char const *argv[]) {
	...
	vector<int> arr{10,0,8,3,6,1,4,2};
